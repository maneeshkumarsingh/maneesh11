...
trainingData = config['TRAINING_DATA_PATH'] #['Model_configration']
#add_trainingData = config['Model_configration']['Add_TrainingData']
...
def train():
    ...
    try:
        ...
        sparse_vectors = scipy.sparse.csr_matrix(X_ngrams)

        A2P_P2P_detect_model = RandomForestClassifier(n_estimators=100).fit(sparse_vectors, training_df['labels'])
        print('A2P_P2P_detect_model_done')
